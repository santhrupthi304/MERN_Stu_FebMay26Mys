// functions for movies handling are created
...
function getAllMovies(req,res){
    ...
    if(language){
        filterMovies = filterMovies.filter((movie)=>movie.language.toLowerCase()=== language.toLowerCase());
    }

    if(genre){
        filterMovies = filterMovies.filter((movie)=>movie.genre.toLowerCase()=== genre.toLowerCase());
    }
    if(city){
        filterMovies = filterMovies.filter((movie)=>movie.city.toLowerCase()=== city.toLowerCase());
    }
...
function deletedMovie(req,res,next){
    const movieId = Number(req.params.id);
